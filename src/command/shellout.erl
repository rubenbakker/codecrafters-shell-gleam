...
-export([display/1, color/1, background/1, arguments/0, command/5, exit/1, which/1, style/3]).
...
-spec command(binary(), list(binary()), binary(), binary(), list(command_opt())) -> {ok,
        binary()} |
    {error, {integer(), binary()}}.
command(Executable, Arg0, Arguments, Directory, Options) ->
